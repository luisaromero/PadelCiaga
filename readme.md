# Padel Ciaga 🎾

**Padel Ciaga** es una tienda online de ropa deportiva creada **por y para mujeres que disfrutan del pádel**. Un proyecto enfocado en combinar comodidad, rendimiento y diseño contemporáneo, con una estética minimalista inspirada en marcas deportivas femeninas.

La plataforma permite explorar productos, visualizar disponibilidad, agregar artículos al carrito y gestionar una experiencia de compra sencilla y visual.

## 🚀 Proyecto desplegado

El proyecto se encuentra desplegado en **Render**.

> Nota: Al estar alojado en un servicio gratuito, el servidor puede entrar en modo inactivo. Si al ingresar la página no carga a la primera, espera unos segundos y vuelve a intentarlo para activar nuevamente el servidor.

🔗 **Demo:** 
## Link :

*https://padelciaga.onrender.com/*


## Tecnologías utilizadas

* HTML5
* CSS3
* Bootstrap
* JavaScript
* Node.js
* Express
* Handlebars
* Render (Deploy)

## Funcionalidades principales

* Diseño responsive adaptado a dispositivos móviles.
* Renderizado dinámico de productos con Handlebars.
* Backend con Node.js y Express..
* Formulario de contacto.
* Manejo de disponibilidad de productos.
* Organización mediante layouts y partials.

---

Proyecto desarrollado como parte de mi portafolio de desarrollo web.



